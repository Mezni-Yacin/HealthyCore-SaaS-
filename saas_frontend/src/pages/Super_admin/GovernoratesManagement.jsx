...
export default function GovernoratesManagement() {
  ...
  const paginatedData = governorates.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);

  const openCreateModal = () => {
    setEditingGov(null); setFormName(''); setFormCode(''); setFormErrors({});
    setShowModal(true);
  };

  const openEditModal = (gov) => {
    setEditingGov(gov); setFormName(gov.name); setFormCode(gov.code); setFormErrors({});
    setShowModal(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = { name: formName.trim(), code: formCode.trim().toUpperCase() };
    ...
    try {
      ...
      setMessageType('success'); setShowModal(false); fetchGovernorates();
    } catch (err) {
  ...
  const handleDelete = async (gov) => {
    if (!window.confirm(`Supprimer le gouvernorat "${gov.name}" ?`)) return;
    try {
      await api.delete(`/users/governorates/${gov.id}/`);
      setMessage(`Gouvernorat "${gov.name}" supprimé.`); setMessageType('success'); fetchGovernorates();
    } catch (err) {
  ...
  return (
    <div className="container-fluid py-4">
      <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
        <div>
          <h2 className="fw-bold mb-1"><i className="bi bi-map me-2 text-primary"></i>Gestion des Gouvernorats</h2>
          <p className="text-muted mb-0">{governorates.length} gouvernorat{governorates.length > 1 ? 's' : ''} au total</p>
        </div>
        ...
        <div className="card-body">
          <div className="row g-2 align-items-center">
            ...
            <div className="col-md-4 text-md-end">
              <small className="text-muted">Tri : {ordering === 'name' ? 'Nom A-Z' : ordering === '-name' ? 'Nom Z-A' : ordering === 'code' ? 'Code A-Z' : ordering === '-code' ? 'Code Z-A' : 'ID'}</small>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="card shadow-sm border-0">
        <div className="card-body p-0">
          {loading ? (
            ...
            <div className="text-center py-5"><i className="bi bi-inbox display-1 text-muted"></i><p className="mt-2 text-muted">Aucun gouvernorat trouvé.</p></div>
          ) : (
            <>
                ...
                <table className="table table-hover align-middle mb-0">
                  <thead className="table-light">
                    <tr>
                      ...
                      <th style={{ cursor: 'pointer' }} onClick={() => handleOrderToggle('code')}>Code <SortIcon field="code" /></th>
                  ...
                  <tbody>
                    {paginatedData.map((gov, index) => (
                      <tr key={gov.id}>
                        <td className="ps-3 text-muted">{(currentPage - 1) * itemsPerPage + index + 1}</td>
                        <td className="fw-semibold">{gov.name}</td>
                        <td><span className="badge bg-secondary bg-opacity-10 text-secondary">{gov.code}</span></td>
                        <td className="text-center">
                          <div className="btn-group btn-group-sm">
                            <button className="btn btn-outline-primary" onClick={() => openEditModal(gov)} title="Modifier"><i className="bi bi-pencil"></i></button>
                            <button className="btn btn-outline-danger" onClick={() => handleDelete(gov)} title="Supprimer"><i className="bi bi-trash"></i></button>
                          </div>
                        </td>
                      </tr>
                    ))}
              ...
              <div className="card-footer bg-white border-top">
                <div className="d-flex justify-content-between align-items-center">
                  <small className="text-muted">Affichage {(currentPage - 1) * itemsPerPage + 1}–{Math.min(currentPage * itemsPerPage, governorates.length)} sur {governorates.length}</small>
      ...
      >
        <FormField label="Nom du gouvernorat" required error={formErrors.name} helpText="Nom unique (min 2 caractères)">
        ...
        <FormField label="Code" required error={formErrors.code} helpText="Code unique, max 10 caractères (majuscules auto)">
          <input type="text" className={`form-control ${formErrors.code ? 'is-invalid' : ''}`} value={formCode} onChange={(e) => setFormCode(e.target.value.toUpperCase())} maxLength={10} style={{ textTransform: 'uppercase' }} />
        </FormField>
      </Modal>
    </div>
  );
