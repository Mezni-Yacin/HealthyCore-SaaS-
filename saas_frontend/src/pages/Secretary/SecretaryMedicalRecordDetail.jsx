...
export default function SecretaryMedicalRecordDetail() {
  const { id } = useParams();
  ...
  const fetchRecord = async () => {
    try {
      ...
      const res = await api.get(`/medical-records/secretary/${id}/`);
      setRecord(res.data);
    } catch (err) {
  ...
  if (error) return <div className="container py-4"><div className="alert alert-danger">{error}</div><button className="btn btn-outline-primary" onClick={() => navigate('/secretary-records')}><i className="bi bi-arrow-left me-2"></i>Retour</button></div>;
  ...
  return (
    <div className="container-fluid py-4">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          <button className="btn btn-outline-secondary btn-sm me-2" onClick={() => navigate('/secretary-records')}><i className="bi bi-arrow-left"></i></button>
        ...
        <div className="card-body">
          <div className="row">
            <div className="col-md-3">
              <h6 className="text-muted mb-1">Patient</h6>
              <h5>{record.patient_info?.full_name || '—'}</h5>
              {record.patient_info?.phone_number && <small className="text-muted"><i className="bi bi-telephone me-1"></i>{record.patient_info.phone_number}</small>}
            </div>
            <div className="col-md-3">
            ...
            <div className="col-md-1"><h6 className="text-muted mb-1">Priorité</h6><p className="mb-0">{getPriorityBadge(record.priority)}</p></div>
            <div className="col-md-2"><h6 className="text-muted mb-1">Confidentialité</h6><p className="mb-0"><span className="badge bg-light text-dark">{record.confidentiality_display || record.confidentiality_level}</span></p></div>
          </div>
        </div>
      </div>
