...
const calcTotal = (subtotal, tax, discount, cnam, insurance) => {
  const s = parseFloat(subtotal) || 0; const t = parseFloat(tax) || 0;
  const d = parseFloat(discount) || 0; const c = parseFloat(cnam) || 0; const ins = parseFloat(insurance) || 0;
  return Math.max(0, (s + t - d) - c - ins).toFixed(3);
};

/* ══════════════════ PDF Helper ══════════════════ */
const handleDownloadPdf = async (inv, role = 'secretary') => {
  try {
    const res = await api.get(`/billing/${role}/invoices/${inv.id}/pdf/`, { responseType: 'blob' });
    const url = window.URL.createObjectURL(new Blob([res.data], { type: 'application/pdf' }));
    const link = document.createElement('a');
    link.href = url; link.setAttribute('download', `Facture_${inv.invoice_number || inv.id}.pdf`);
    document.body.appendChild(link); link.click(); link.parentNode.removeChild(link);
  } catch (err) { console.error("Erreur PDF", err); alert("Impossible de télécharger le PDF."); }
};
...
const SecretaryInvoices = () => {
  ...
  const [createForm, setCreateForm] = useState({
    appointment: '', patient_name: '', doctor: '', doctor_name: '', subtotal: '', tax_amount: '',
  ...
  const fetchAll = useCallback(() => {
    api.get('/billing/secretary/stats/').then(r => setStats(r.data)).catch(() => {});
    setLoading(true);
    api.get('/billing/secretary/invoices/', { params: { page: 1, page_size: 50 } })
  ...
  /* ════════════ Create ════════════ */
  const openCreate = () => {
    setCreateForm({ appointment: '', patient_name: '', doctor: '', doctor_name: '', subtotal: '', tax_amount: '', discount_amount: '0', cnam_contribution: '0', insurance_contribution: '0', due_date: '', payment_method: '', notes: '' });
    setCreateError(''); setApptSearch(''); setShowCreate(true); setApptLoading(true);
    api.get('/appointments/secretary/', { params: { page_size: 200, ordering: '-start_time' } })
  ...
  const handleApptSelect = (apptId) => {
    ...
    if (appt) {
      setCreateForm(p => ({ ...p, appointment: appt.id, patient_name: appt.patient_name || `${appt.patient_first_name || ''} ${appt.patient_last_name || ''}`.trim(), doctor: appt.doctor || appt.doctor_id || '', doctor_name: appt.doctor_name || `${appt.doctor_first_name || ''} ${appt.doctor_last_name || ''}`.trim() }));
    } else {
      setCreateForm(p => ({ ...p, appointment: '', patient_name: '', doctor: '', doctor_name: '' }));
    }
  };
  ...
  const handleCreate = (e) => {
    ...
    api.post('/billing/secretary/invoices/', payload)
  ...
  /* ════════════ Actions ════════════ */
  const handleValidate = (inv) => { if (!window.confirm('Valider la facture ' + inv.invoice_number + ' ?')) return; api.post('/billing/secretary/invoices/' + inv.id + '/validate/').then(() => fetchAll()).catch(() => {}); };
  const handleDelete = (inv) => { if (!window.confirm('Supprimer la facture ' + inv.invoice_number + ' ?')) return; api.delete('/billing/secretary/invoices/' + inv.id + '/').then(() => { setShowDetail(false); fetchAll(); }).catch(() => {}); };
  ...
  const handlePay = (e) => {
    ...
    api.post('/billing/secretary/invoices/' + payTarget.id + '/pay/', payload).then(() => { setShowPay(false); setPayTarget(null); fetchAll(); }).catch(() => {}).finally(() => setPayLoading(false));
  };

  const filteredAppointments = appointmentsList.filter(a => {
    if (!apptSearch) return true; const q = apptSearch.toLowerCase();
    const pName = (a.patient_name || `${a.patient_first_name || ''} ${a.patient_last_name || ''}`).toLowerCase();
    const dName = (a.doctor_name || `${a.doctor_first_name || ''} ${a.doctor_last_name || ''}`).toLowerCase();
    const reason = (a.reason || a.notes || a.motif || '').toLowerCase();
    return pName.includes(q) || dName.includes(q) || reason.includes(q) || String(a.id).includes(q);
  });
  ...
  return (
    <div className="container-fluid py-4">
      <div className="d-flex flex-wrap align-items-center justify-content-between mb-4">
        <div>
          <h4 className="mb-1"><i className="bi bi-receipt me-2 text-primary"></i>Facturation du Cabinet</h4>
          <p className="text-muted mb-0">Gestion des factures du cabinet</p>
        </div>
      ...
      <div className="card border-0 shadow-sm">
        <div className="card-header bg-white d-flex justify-content-between align-items-center py-3">
          <h6 className="mb-0"><i className="bi bi-list-ul me-2"></i>Factures du cabinet</h6>
        ...
        <div className="card-body p-0">
          {loading ? (<div className="text-center py-5"><div className="spinner-border text-primary"></div></div>) : invoices.length === 0 ? (<div className="text-center py-5 text-muted"><i className="bi bi-inbox fs-1 d-block mb-2 opacity-50"></i><p>Aucune facture</p></div>) : (
            <div className="table-responsive">
              <table className="table table-hover mb-0">
                <thead className="table-light"><tr><th>N° Facture</th><th>Patient</th><th>Médecin</th><th>Montant</th><th>Payé</th><th>Reste</th><th>Statut</th><th>Actions</th></tr></thead>
                <tbody>
                  {invoices.map(inv => (
                    <tr key={inv.id}>
                      ...
                      <td>
                        <div className="d-flex gap-1">
                          ...
                          {/* BOUTON PDF TABLEAU */}
                          <button className="btn btn-sm btn-dark" onClick={() => handleDownloadPdf(inv, 'secretary')} title="Télécharger PDF"><i className="bi bi-file-earmark-pdf-fill"></i></button>
                        </div>
                      </td>
                    </tr>
                  ))}
          ...
          <div className="modal-dialog modal-lg modal-dialog-centered">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title"><i className="bi bi-receipt me-2"></i>{selected.invoice_number} <span className={'badge bg-' + (STATUS_MAP[selected.status] || 'secondary') + ' ms-2'}>{selected.status_display || selected.status}</span></h5>
                <button type="button" className="btn-close" onClick={() => setShowDetail(false)}></button>
              </div>
              <div className="modal-body">
                <div className="row g-3">
                  <div className="col-md-4"><div className="card border"><div className="card-body"><small className="text-muted">Patient</small><p className="fw-bold mb-0">{selected.patient_name || '-'}</p></div></div></div>
                  <div className="col-md-4"><div className="card border"><div className="card-body"><small className="text-muted">Médecin</small><p className="fw-bold mb-0">{selected.doctor_name || '-'}</p></div></div></div>
                  <div className="col-md-4"><div className="card border"><div className="card-body"><small className="text-muted">Total</small><p className="fw-bold mb-0 text-primary fs-5">{fmt(selected.total_amount)}</p></div></div></div>
                </div>
              ...
              <div className="modal-footer">
                ...
                {/* BOUTON PDF MODAL */}
                <button className="btn btn-dark" onClick={() => handleDownloadPdf(selected, 'secretary')}><i className="bi bi-file-earmark-pdf-fill me-1"></i>Télécharger PDF</button>
            ...
            <div className="modal-content">
              ...
              <div className="modal-body">
                <div className="mb-4">
                  ...
                  <input type="text" className="form-control mb-2" placeholder="Rechercher par patient, médecin..." value={apptSearch} onChange={e => setApptSearch(e.target.value)} />
                  {apptLoading ? (<div className="text-center py-3"><div className="spinner-border spinner-border-sm text-primary"></div></div>) : (
                    <select className="form-select" size="5" value={createForm.appointment} onChange={e => handleApptSelect(e.target.value)} style={{ cursor: 'pointer' }}>
                      ...
                      {filteredAppointments.map(a => { const pName = a.patient_name || `${a.patient_first_name || ''} ${a.patient_last_name || ''}`.trim(); const dName = a.doctor_name || `${a.doctor_first_name || ''} ${a.doctor_last_name || ''}`.trim(); const dateStr = a.date || a.start_time ? new Date(a.date || a.start_time).toLocaleDateString('fr-FR') : ''; const time = a.start_time || a.time_slot ? new Date(a.start_time || a.time_slot).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' }) : ''; const reason = a.reason || a.notes || a.motif || 'Consultation'; return (<option key={a.id} value={a.id}>Dr. {dName} — {pName} — {dateStr} {time} — {reason}</option>); })}
                    </select>
                  )}
                </div>

                {selectedAppt && (<div className="alert alert-light border mb-4"><div className="row g-2"><div className="col-sm-3"><small className="text-muted">Patient</small><div className="fw-semibold">{createForm.patient_name || '-'}</div></div><div className="col-sm-3"><small className="text-muted">Médecin</small><div className="fw-semibold text-primary"><i className="bi bi-person-badge me-1"></i>{createForm.doctor_name || '-'}</div></div><div className="col-sm-3"><small className="text-muted">Date & Heure</small><div className="fw-semibold">{selectedAppt.date || selectedAppt.start_time ? new Date(selectedAppt.date || selectedAppt.start_time).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' }) : '-'} {selectedAppt.start_time ? new Date(selectedAppt.start_time).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' }) : ''}</div></div><div className="col-sm-3"><small className="text-muted">Motif</small><div className="fw-semibold">{selectedAppt.reason || selectedAppt.notes || selectedAppt.motif || 'Consultation'}</div></div></div></div>)}
                ...
                <form onSubmit={handleCreate}>
                  ...
                  <div className="row g-3 mb-3">
                    <div className="col-md-3"><label className="form-label">CNAM (TND)</label><input type="number" step="0.001" min="0" className="form-control" placeholder="0.000" value={createForm.cnam_contribution} onChange={e => setCreateForm(p => ({ ...p, cnam_contribution: e.target.value }))} /></div>
                    <div className="col-md-3"><label className="form-label">Assurance (TND)</label><input type="number" step="0.001" min="0" className="form-control" placeholder="0.000" value={createForm.insurance_contribution} onChange={e => setCreateForm(p => ({ ...p, insurance_contribution: e.target.value }))} /></div>
                    <div className="col-md-3"><label className="form-label">Total calculé</label><div className="form-control bg-primary bg-opacity-10 fw-bold text-primary" style={{ fontSize: '1.1rem' }}>{computedTotal} TND</div></div>
                    <div className="col-md-3"><label className="form-label">Échéance</label><input type="date" className="form-control" value={createForm.due_date} onChange={e => setCreateForm(p => ({ ...p, due_date: e.target.value }))} /><small className="text-muted">30j si vide</small></div>
                  </div>
                  <div className="row g-3 mb-3">
                    ...
                    <div className="col-md-6"><label className="form-label">Notes</label><input type="text" className="form-control" placeholder="Notes internes" value={createForm.notes} onChange={e => setCreateForm(p => ({ ...p, notes: e.target.value }))} /></div>
                  </div>
